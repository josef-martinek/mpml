from examples.mlmc_lognormal import LognormalPDESample as sample_cls
...
mse_tol_array = [8e-6, 4e-6, 2e-6, 1e-6]
num_runs_per_tolerance = 1000
num_workers = 72
init_nsamples = 50

# Number of times to run the simulation
output_folder = "data/mlmc_minres_fixtol1e-6/"
...
def run_simulation(run_id, mse_tol, model, Lmin, Lmax, alpha, beta, mse_tol_id):
    """
    ...
    try:
        ...
        algorithm.run(mse_tol=mse_tol, init_nsamp=init_nsamples)
        end_time = time.time()
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        results = list(executor.map(
            functools.partial(run_simulation, mse_tol=mse_tol, model=model, 
                              Lmin=Lmin, Lmax=Lmax, alpha=alpha, beta=beta, mse_tol_id=mse_tol_array.index(mse_tol)),
